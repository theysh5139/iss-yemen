...
export default function Dashboard() {
  ...
  // Redirect admin users to admin dashboard, members to homepage
  useEffect(() => {
    if (user?.role === 'admin') {
      navigate("/admin/dashboard", { replace: true })
    } else if (user?.role === 'member') {
      navigate("/", { replace: true }) // Members see homepage as their dashboard
    }
  }, [user, navigate])
                ...
                <div className="events-list">
                  {registeredEvents.map(event => (
                    <div key={event._id} className="event-item card-3d">
                      ...
                      <a href={`/events#${event._id}`} className="event-item-link">
                        View Details →
                      </a>
                    </div>
                  ))}
